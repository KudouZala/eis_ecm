...
def set_plot_style(
    ...
    #"""修改默认的arviz/matplotlib配置以获得更漂亮的图"""
    # Arviz
    if use_arviz:
    ...
    try:
        ...
        pass
    """
    这段代码的目的是为了设置Jupyter Notebook中的matplotlib图形显示格式，使其具有更高的分辨率（Retina显示效果）。这主要是针对那些支持Retina显示的设备，例如某些MacBook或其他高分辨率屏幕的笔记本电脑。
    代码解释：
    try语句块：尝试执行一些可能会因为各种原因而失败的代码。在这种情况下，它尝试导入IPython模块并设置matplotlib的显示格式。
    import IPython：尝试导入IPython模块，它是Jupyter Notebook的一部分，用于提供丰富的交互功能。
    IPython.display.set_matplotlib_formats("retina")：如果IPython模块导入成功，这行代码会将matplotlib的输出格式设置为“retina”。这意味着所有通过matplotlib生成的图形都会有更高的像素密度，从而在Retina显示屏上看起来更清晰。
    except ImportError：如果在尝试执行try块中的代码时发生了ImportError异常（通常是因为找不到或无法导入某个模块），这个except块会被执行。
    pass：pass是一个空操作，什么也不做。在这里，它的意思是如果IPython模块无法导入（比如不在Jupyter环境中或者其他原因），程序不会做任何事情，也不会因为这个导入错误而中断或抛出异常。
    简而言之，这段代码是一个预防措施，确保如果在Jupyter环境中运行，且支持Retina显示格式，就使用高分辨率显示；如果不是在这样的环境中或者不支持，代码也不会产生负面影响。
    """
