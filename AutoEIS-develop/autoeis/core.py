...
def preprocess_impedance_data(
    ...
    #预处理阻抗数据，过滤出高频区域中虚部为正的数据并应用KK验证。
    Parameters
    ----------
    impedance : np.ndarray[complex]
        Impedance measurements.阻抗测量值。
    freq : np.ndarray[float]
        Frequencies corresponding to impedance measurements.阻抗测量值对应的频率值。
    threshold : float
        Controls the filtering effect during KK validation.#控制KK验证过程中过滤效果的阈值。
    plot : bool, optional
        If True, a plot of the processed data will be generated. Default is False.若为True，则生成处理后数据的图表。默认为False。

    Returns返回值:
    -------
    tuple
        - Z: Filtered impedance data.过滤后的阻抗数据。
        - freq: Frequencies corresponding to the filtered measurements.对应过滤后测量值的频率。
        - rmse: Root mean square error of KK validated data vs. measurements.KK验证的数据与测量值之间的均方根误差。
    """
...
def _perform_bayesian_inference_batch(
    ...
    """
    # 如果需要，用标准的 Pool 替换 WorkerPool
    results=[]
    # 使用包装函数确保正确地传递参数
    with Pool(processes=n_jobs) as pool:
        if progress_bar:
            # 为每个电路创建一个参数元组，确保包括 freq 和 Z
            args_list = [(circuit, freq, Z, p0_) for circuit, p0_ in zip(circuits, p0)]

            # 使用 tqdm 在终端显示进度条
            for result in tqdm(pool.imap(_perform_bayesian_inference_wrapper, args_list), total=len(circuits),
                               desc="执行贝叶斯推断"):
            ...
            args_list = [(circuit, freq, Z, p0_) for circuit, p0_ in zip(circuits, p0)]
            # 直接映射没有进度条
            results = pool.map(_perform_bayesian_inference_wrapper, args_list)
    ...
    #
    # return results
def _perform_bayesian_inference_wrapper(args):
    """一个包装函数，用来解包参数并调用 _perform_bayesian_inference。"""
    return _perform_bayesian_inference(*args)
